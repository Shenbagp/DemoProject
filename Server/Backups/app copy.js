...
let fetchRecords = async () => {
    const res = await Model.find({}, (err, docs) => console.log(err, docs))
}

let fetchOneRecords = async () => {
    const res = await Model.findOne({ "name": "Shen" }, (err, docs) => console.log("fetch One record", err, docs))
}

//Stocks Schema
let fetchstockRecords = async () => {
    const res = await StockModel.find({ "Company": { "$regex": "AA" } }, (err, docs) => console.log("Stocks Schema - ", err, docs))
}

// only company name

let fetchCompanyName = async () => {
    const res = await StockModel.find({}, { Company: 1 }, (err, docs) => { console.log(docs) })
}
